...
def build(config: AppConfig, *, allow_fit: bool = True):
    ...
    from gaia_dr4_explorer.data.chain import ProductChain
    ...
    products = ProductChain(BundledProductProvider(), GaiaArchiveProvider(config))
    return build_app(provider, allow_fit=allow_fit, archive=products)
